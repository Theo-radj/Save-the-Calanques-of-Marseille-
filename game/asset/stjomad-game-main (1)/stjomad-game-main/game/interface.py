    ...
    while ecran_de_fin :
      ...
      if play_button_rect.collidepoint(mouse_x, mouse_y) and pygame.mouse.get_pressed()[0] or pygame.key.get_pressed()[pygame.K_SPACE]:
        ...
        while self.level == None:
            self.ecran.fill((0,0,0))
            level1 = self.police.render("Level 1",True , (255,255,255))
            self.ecran.blit(level1,(level1.get_rect(center = (self.size[0]//2, 200 ))))
            level2 = self.police.render("Level 2",True , (255,255,255))
            self.ecran.blit(level2,(level2.get_rect(center = (self.size[0]//2, 300 ))))
            level3 = self.police.render("Level 3",True , (255,255,255))
            self.ecran.blit(level3,(level3.get_rect(center = (self.size[0]//2, 400 ))))
            pygame.display.flip()
